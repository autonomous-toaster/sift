...
fn compact_string(s: &str, max_string_len: usize) -> serde_json::Value {
...
fn compact_object(
    ...
    let entries: Vec<(String, serde_json::Value)> = obj
        ...
        .map(|(k, v)| {
            (
                k.clone(),
                compact_value(v, max_string_len, max_array_items, max_depth, max_keys, depth + 1),
            )
        })
        .collect();
    let mut result = serde_json::Map::from_iter(entries);
    if obj.len() > max_keys {
        let remaining = obj.len() - max_keys;
        result.insert("...".to_string(), serde_json::Value::String(format!("+{remaining} more")));
    }
...
fn compact_value(
    ...
    match val {
        serde_json::Value::String(s) => compact_string(s, max_string_len),
        serde_json::Value::Array(arr) => compact_array(arr, max_string_len, max_array_items, max_depth, max_keys, depth),
        serde_json::Value::Object(obj) => compact_object(obj, max_string_len, max_array_items, max_depth, max_keys, depth),
        other => other.clone(),
    }
}
