...
return {
    ...
    execute = function(ctx, args, stdin)
        ...
        if sift.cache.has_file(ctx, hash) then
            ...
            return {
                status = "unchanged",
                message = string.format("[nudge] %s lines %d-%d unchanged — already in your context. command sed -n '%d,%dp' %s to re-read.", display_name, range.start, range_end, range.start, range_end, path),
                raw_bytes = stat.size
            }
        end
